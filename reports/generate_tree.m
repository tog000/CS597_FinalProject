...
all_examples = zeros(1,length(indicators)+1);
    ...
    for i=1:length(crime_indices)
        for ind=1:length(indicators)
            ...
            if length(file_list) >= 1
                ...
                [h w] = size(data);
                if h > 20
                    all_examples(start_index+i,ind) = mean(sorted(1:5,3));
                end
            end
        end
        all_examples(start_index+i,length(indicators)+1) = ct;
    end
end

%indices_nan=isnan(all_examples);
sum2 = sum(all_examples(:,1:end-1),2);
%[h w] = size(all_examples);
...
examples = all_examples(:,1:end-1);
classes = all_examples(:,end);
...
tc = ClassificationTree.fit(examples,classes,'PredictorNames',indicators,'minleaf',2);
view(tc,'mode','graph') 
